...
async function CountdownTimer(duration: number) {
  ...
  let remainingSeconds = setInterval(() => {
    ...
    if (remaining <= 0) {
      ...
      console.log(
        chalk.bgRed.white.italic.underline(
  ...
  const { duration } = await inquirer.prompt([
    {
      type: "number",
      name: "duration",
      message: chalk.white.bold.underline(
        "Enter the duration of the countdown in seconds:"
      ),
